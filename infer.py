...
def main():
    ...
    # Data loader
    # cloth_test_dataset = LIPDataValSet(args.data_dir, 'val', crop_size=input_size, transform=transform, flip=args.flip)
    cloth_test_dataset = ClothDataset(args.data_dir, 'test', crop_size=input_size, transform=transform)
    num_samples = len(cloth_test_dataset)
    print('Totoal testing sample numbers: {}'.format(num_samples))
    testloader = data.DataLoader(cloth_test_dataset, batch_size=args.batch_size, shuffle=False, pin_memory=True)
